...
public class FinalFieldTest {
    ...
    *   따라서 초기 인스턴스 생성 후 기본값 0이 필드에 들어가는데
    *   그 초기화 이후 값을 변경하지 못해서 선언하면서 바로 초기화(값 넣기)를 해줘야 한다.
...
//    private static final int STATIC_NUM;
    ...
    private static final int STATIC_NUM=1;      //static은 생성자 초기화 불가, 이미 값이 들어가 있기 때문 ex. 한 반의 에어컨 1개, 22명이라고 22개가 아니고

    /* 필기.
    *   생성자를 이용한 초기화는 불가능하다.
    *   생성자는 인스턴스가 생성되는 시점에 호출 되기 때문에 그전에는 초기화가 일어나지 못한다.
    *   하지만 static은 프로그램이 start될 때 할당되기 때문에 초기화가 되지 않은 상태로 선언된 것과 동일하여
    *   기본값으로 초기화 된 후 값을 변경하지 못하기 때문에 에러가 발생한다.
    *   new를 쓰지 않아도 프로그램이 동작하자 마자 바로 실행된 것
    *  */
...
//    }
    ...
    static {
        STATIC_DOUBLE=0.5;      //프로그램이 시작할 때 static을 먼저 쭉 훑는 데 이때 0.5로 넣어주게 됨
    }
