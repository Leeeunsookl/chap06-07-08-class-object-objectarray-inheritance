...
public class User {
    ...
    *   */
    ...
    /* 목차. 1. 기본 생성자(default constructor) */        //void 등을 안쓰는 것은 메소드와 다르다는 것
    public User() {
    ...
    /* 필기. 매개변수 있는 생성자의 주 목적은 인스턴스 생성 시점에 매개변수로 전달받은 값을 이용해서
    *       받은 값을 이용해서 필드를 초기화 하는 목적이다. */
    public User( String id, String pwd, String name){
        ...
        System.out.println("User 클래스의 id, pwd, name 초기화 하는 생성자 호출함");
    }
...
//        this.enrollDate=enrollDate;
        ...
        *   괄호 안에 매개변수의 타입, 갯수, 순서에 맞는 생성자를 호출하고 복귀 (메소드와 동일)
        *   this()는 가장 첫 줄에 선언해야 하며, 그렇지 않는 경우 Compile Error가 발생한다.
        *  */
        this(id, pwd, name);        //미리 작성한 세 개의 필드를 초기화(setting) 하는 생성자로 매개변수로 받은 값을 전달.
        this.enrollDate = enrollDate;
        System.out.println("User 클래스의 모든 필드를 초기화 하는 생성자 호출");
    }
    public String getInfo(){
        return "[User id=]"+ this.id + " [this.pwd=]" + this.pwd + " [this.name]=" + this.name +" [enrollDate]=" +this.enrollDate;
    }
    }
