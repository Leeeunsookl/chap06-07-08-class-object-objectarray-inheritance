...
public class ParameterTest {
    ...
    public void testParameterPrimaryTypeArray(int[] iarr){
        ...
        /* 필기.
        *   배열의 주소가 전달 될 것이다.
        *   즉 인자로 전달하는 배열과 매개별수로 전달 받은 배열은 서로 동일한 배열을 가리킨다. (얕은 복사)
        *  */
    ...
    public void testParameterClassType(RectAngle rectAngle){        //r1 레퍼런스 변수를 rectAngle 레퍼런스 변수로 집어넣음

        /* 필기.
        *   인스턴스도 주소가 전달된다.
        *   즉, 인자로 전달하는 인스턴스와 매개변수로 전달받은 동일한
        *   인스턴스를 가리킨다. (얕은 복사)
        *  */
    ...
    public void testVariableLengthArrayParameter(String name, String...hobby){
        //... 이 가변인자라는 것
        /* 필기
        *   가변길이의 배열은 몇 개가 매개변수로 전달 될 지 모르는 상황이기 때문에
        *   이름과 구분하기 위해 뒤쪽에 작성해야 한다. (언제 끝날지 몰라서 준비가 안됨)
