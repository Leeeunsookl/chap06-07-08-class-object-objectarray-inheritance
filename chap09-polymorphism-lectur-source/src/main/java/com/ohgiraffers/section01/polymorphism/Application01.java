...
public class Application01 {
    ...
    public static void main(String[] args) {
        ...
        *   그렇기 때문에 하나의 타입으로 여러 타입이 인스턴스 처리할 수 있기도 하고,
        *   하나의 메소드 호출로 객체별로 각기 다른 방법으로 동작할 수도 있다.
        *   */

        /* 필기.
         *   다형성이란, 객체지향 프로그램의 3대 특징 캡슐화, 상속, 다형성 중 하나이며
         *   객체지향 프로그래밍의 꽃이라고 불리울 정도로 활용성이 높고 장점이 있다.
         *  */

        /* 필기.
         *   다형성 장점.
         *   1. 여러 타입 객체를 하나의 타입으로 관리할 수 있으므로 유지보수성, 생산성 증가
         *   2. 상속기반 기술이므로 상속관계(연관)에 있는 객체는 동일한 메세지(메소드)를
         *      수신할 수 있다.
        ...
        /* 필기.
        *   토끼랑 호랑이는 동물 클래스를 상속받았다.
        *   따라서 토끼는 토끼 타입이기도 하면서 동물 타입이다.
        *   호랑이도 마찬가지로 호랑이 타입이기도 하면서 동물 타입이다.
        *  */

        /* 목차. 5. 부모 타입으로(동물)으로 자식 인스턴스(호랑이, 토끼) 주소값 저장 */
...
//        토끼 r1 = new 동물();
//        호랑이 t1 = new 동물();   //더 작은 값은 안됨

        /* 목차. 6. 동적 바인딩 확인
        *   동적 바인딩
        *   컴파일(번역) 당시 해당 타입의 메소드와 연결되어 있다가
        *   런타임(프로그램 실행) 당시 실제 객체가 가진 오버라이딩 메소드로 바인딩 바뀌어서 동작하는 것을 의미
        *  */
        ...
        *       토끼와 호랑이가 가지고 있는 고유한 기능은 작동시키지 못한다.
        *  */
//        a1. 점프();
//        a2. 물어뜯기();       //부모돈도 내돈 , 내돈은 내돈 : 그래서 자식 클래스 고유 메소드는 안됨

        /* 목차. 7. 타입 형변환 확인 */
        /* 필기.
        *   객체별 고유한 기능(점프, 물어뜯기)를 통작시키기 위해서는 형변환을 통해 토끼와 호랑이로 변경해야
        *   메소드 호출이 가능하다.
...
//        ((호랑이)a1).물어뜯기();     //RuntimeExeption 번역은 문제가 없지만, 프로그램 동작 시 오류 발생
        ...
        *  */
        ...
        /* 목차. 8. instanceof 연산자 사용 */
        /* 필기.
        *   클래스 형변환은 up-casting과 down-casting 으로 구분할 수 있다.
...
//        토끼 r3 = a3;     //토끼를 int로 a3를 double이라고 생각하면 된다. down-casting 묵시적 형변환 불가
        토끼 r4 = (토끼)a3;  //down-casting 명시적 형변환

        //슈퍼클래스가 타입, 서브클래스가 인스턴스
